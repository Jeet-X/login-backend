...
class CategoryService {
    ...
    async getAllCategoriesForMobile() {
        try {
            const subcategories = await categoryModel.getAllWithSubCategories();
            ...
            //                 // const tournaments = await tournamentSlotModel.findActiveSlots(subCategory.id, req?.user?.id);
    ...
    async getCategoryDetails(categoryId) {
        try {
            const subCategories = await categoryModel.getCategoryWithStats(categoryId);
