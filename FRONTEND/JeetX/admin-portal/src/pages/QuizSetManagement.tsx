...
const QuizSetManagement: React.FC = () => {
    ...
    const handleCategoryChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        setFormData(prev => ({ ...prev, sub_category_id: '', fixed_question_ids: [] }));
        setAvailableQuestions([]);
    };

    const handleSubCategoryChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
        const subId = e.target.value;
        setFormData(prev => ({ ...prev, sub_category_id: subId, fixed_question_ids: [] }));
        if (subId) {
            fetchAvailableQuestions(subId);
        }
    };
    ...
    const handleEdit = (set: QuizSet) => {
        ...
        // Find category to populate dropdowns
        const parentCat = categories.find(cat => cat.sub_categories.some(sub => sub.id === set.sub_category_id));
        if (parentCat) {
            setSelectedCategoryId(parentCat.id);
            setSubCategories(parentCat.sub_categories);
        }
                                ...
                                >
                                    ...
                                    <div className="card-footer-info">
                                        ...
                                        <div className="meta-info">
                                            ...
                                            <span>
                                                {allSubCategories.find(s => s.id === set.sub_category_id)?.name || 'Unknown Category'}
                                            </span>
                                        </div>
                                        <div className="meta-info">
                        ...
                        <div className="form-group">
                            ...
                            >
                                ...
                                {subCategories.length > 0 ? (
                                    ...
                                    <option disabled>{selectedCategoryId ? 'No sub-categories found' : 'Select a category first'}</option>
                                )}
                            </select>
                        </div>
